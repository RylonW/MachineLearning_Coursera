...
for i = 1:m
    realy(i,y(i)) = 1;
J = 1/m*sum(-realy.*log(h3)-(1-realy).*log(1-h3),'all')+lambda/(2*m)* ...
    (sum(Theta1(:,2:end).*Theta1(:,2:end),'all') ...
    +sum(Theta2(:,2:end).*Theta2(:,2:end),'all'));
end
%back
for t = 1:m
    %step1
...
%step5
%T1 = 1 / m * Theta1_grad + lambda/m * Theta1;
%T2 = 1 / m * Theta2_grad + lambda/m * Theta2;
